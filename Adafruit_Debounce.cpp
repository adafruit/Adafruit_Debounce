 ...
 * 
 * @param pin The pin number associated with the button. If pin is < 0 we can use an expander instead of the built in GPIO support.
 * @param polarity The polarity to determine the pressed state (default is LOW which means the input is LOW when the button is pressed).
 */
 ...
 * the button states. This assumes that the pin is GPIO (>= 0), otherwise please use the other begin(bool) which allows you to initialize the button start state.
 */
...
/**
 * @brief Initialize the debounce object with a given initial state. Good for use with expanders.
 * 
...
/**
 * @brief Read the current state of the button. Will call digitalRead if the pin is >= 0, otherwise returns the internal state. 
 * 
 ...
 * 
 * This function updates the button states based on the result of digitalRead(). Not for use with a GPIO expander
 */
...
/**
 * @brief Update the button states with a given state. Good for use with expanders.
 * 
...
/**
 * @brief Check if the button was just pressed since last update(), adjusted for polarity
 * 
...
/**
 * @brief Check if the button was just released since last update(), adjusted for polarity
 * 
